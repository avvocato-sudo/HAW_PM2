...
public class VerleihServiceImplTest
{
    ...
    @Test
    public void testVerlieheneMedienSindVerliehen()
    {
        List<Medium> liste = new ArrayList<>();
        liste.add(_bad);
        liste.add(_shape);
        _verleihService.verleiheAn(_brian,liste,Datum.heute());

        assertTrue(_verleihService.istVerliehen(liste.get(0)));
        assertTrue(_verleihService.istVerliehen(liste.get(1)));

    }

    @Test
    public void testVerleihenMoeglich()
    {
        List<Medium> liste = new ArrayList<>();
        liste.add(_abbey);
        _verleihService.verleiheAn(_brian,liste,Datum.heute());

        List<Medium> nichtVerliehen = new ArrayList<>();
        nichtVerliehen.add(_bad);
        nichtVerliehen.add(_shape);

        assertTrue(_verleihService.istVerleihenMoeglich(_brian, nichtVerliehen));

    }

    @Test
    public void testVerleihNachR�ckgabe()
    {
        List<Medium> liste = new ArrayList<>();
        liste.add(_abbey);
        _verleihService.verleiheAn(_brian,liste,Datum.heute());
        _verleihService.nimmZurueck(liste,Datum.heute());

        assertTrue(_verleihService.istVerleihenMoeglich(_brian, liste));
    }
